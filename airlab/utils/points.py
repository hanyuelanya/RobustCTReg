...
class Points:
    ...
    @staticmethod
    def TRE(points1, points2,sp=None):
        """
        ...
        if sp:
            points1 = np.round(points1/sp)*sp
            points2 = np.round(points2/sp)*sp
            return (np.mean(np.linalg.norm(points1[:n,:]-points2[:n,:], axis=1)), np.std(np.linalg.norm(points1[:n,:]-points2[:n,:], axis=1)))
        else:
            return (np.mean(np.linalg.norm(points1[:n,:]-points2[:n,:], axis=1)), np.std(np.linalg.norm(points1[:n,:]-points2[:n,:], axis=1)))
