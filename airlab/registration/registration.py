...
class PairwiseRegistration(_PairwiseRegistration):
    ...
    def _compute_loss(self, displacement):
        ...
        if self._verbose:
            ...
            for loss_value, loss_name in zip(lossList, loss_names):
                print(
                    str(loss_name) + ": " + str(loss_value.data.item()) + " ",
                    end="",
                    flush=True,
                )
            print("")
        ...
        loss = sum(lossList)
        self.lossIterList.append(loss.clone().detach().cpu())
    ...
    def _closure(self):
        ...
        loss = self._compute_loss(displacement)
...
#         np.save(r"../results"+os.sep+new_filename, np.array(self.lossIterList))


class Old_PairwiseRegistration(_PairwiseRegistration):
    def __init__(self, verbose=True):
        super(PairwiseRegistration, self).__init__(verbose=verbose)
