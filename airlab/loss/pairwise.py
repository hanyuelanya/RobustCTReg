...
class _PairwiseImageLoss(th.nn.modules.Module):
    ...
    def return_loss(self, tensor, welsch_k=1.0):
        if self._size_average and self._reduce:
            return welsch_k * tensor.mean() * self._weight
        if not self._size_average and self._reduce:
            return welsch_k * tensor.sum() * self._weight
        if not self._reduce:
            return welsch_k * tensor * self._weight

class WelschME(_PairwiseImageLoss):
    ...
    def __init__(self, fixed_image, moving_image, vd=5, fixed_mask=None, moving_mask=None, size_average=True, reduce=True):
        super(WelschME, self).__init__(fixed_image, moving_image, fixed_mask, moving_mask, size_average, reduce)
        ...
        self.init_vd = vd
        self.vd = vd
        self.inverse_2_vd_square = 1 / (2 * self.vd ** 2)
        self.firstCalc = True
    ...
    def forward(self, displacement):
        ...
        mask = super(WelschME, self).GetCurrentMask(displacement)
        ...
        if self.firstCalc:
            mes = value.clone().cpu().detach().numpy()
            self.init_vd = np.median(mes)
            self.vd = self.init_vd
            self.inverse_2_vd_square = 1 / (2 * self.vd ** 2)
            self.firstCalc = False
        
        self.value_last = value.clone().detach()
        value =  th.exp(-1 * self.inverse_2_vd_square * self.value_last) * value
        
        return self.return_loss(value,self.inverse_2_vd_square)
...
class WelschNCC(_PairwiseImageLoss):
    ...
    def __init__(self, fixed_image, moving_image,vd=2, fixed_mask=None, moving_mask=None):
        super(WelschNCC, self).__init__(fixed_image, moving_image, fixed_mask, moving_mask, False, True)

        self._name = "welsch_ncc"
        self.isWelschEnabled = True
        self.value_last = -1
        self.init_vd = vd
        self.vd = vd
    ...
    def forward(self, displacement):
        ...
        # compute current mask
        mask = super(WelschNCC, self).GetCurrentMask(displacement)
        ...
        # print(moving_image_valid.shape)
        value = -1. * th.sum(
            (fixed_image_valid - th.mean(fixed_image_valid)) * (moving_image_valid - th.mean(moving_image_valid))) \
                / th.sqrt(th.sum((fixed_image_valid - th.mean(fixed_image_valid)) ** 2) * th.sum(
            (moving_image_valid - th.mean(moving_image_valid)) ** 2) + 1e-10)

        self.value_last = value.clone().detach()
        
        if self.firstCalc:
            nccs = value.clone().cpu().detach().numpy()
            self.init_vd = np.median(nccs)
            self.vd = self.init_vd
            self.inverse_2_vd_square = 1 / (2 * self.vd ** 2)
            self.firstCalc = False
        ...
        return self.return_loss(value,welsch_k=self.inverse_2_vd_square)
...
class NCC(_PairwiseImageLoss):
    ...
    def __init__(self, fixed_image, moving_image, fixed_mask=None, moving_mask=None):
        super(NCC, self).__init__(fixed_image, moving_image, fixed_mask, moving_mask, False, True)
...
class MaskNCC(_PairwiseImageLoss):
    r""" The normalized cross correlation loss is a measure for image pairs with a linear
         intensity relation.
    ...
    def __init__(self, fixed_image, moving_image, fixed_mask=None, moving_mask=None):
        super(MaskNCC, self).__init__(fixed_image, moving_image, fixed_mask, moving_mask, False, True)

        self._name = "ncc"
        self.warped_moving_image = th.empty_like(self._moving_image.image, dtype=self._dtype, device=self._device)
        self.fix = fixed_image.image * fixed_mask.image
        self.mov = moving_image.image * moving_mask.image
    ...
    def forward(self, displacement):
        ...
        # compute current mask
        mask = super(MaskNCC, self).GetMask(displacement)
        self._warped_moving_image = F.grid_sample(self.mov, displacement)
...
class LCC(_PairwiseImageLoss):
    def __init__(self, fixed_image, moving_image, fixed_mask=None, moving_mask=None, sigma=[3], kernel_type="box",
...
class UnoptimizedWelschNGF(_PairwiseImageLoss):
    ...
    def __init__(self, fixed_image, moving_image, vd=0.8, fixed_mask=None, moving_mask=None, epsilon=1e-5,
                 size_average=True, reduce=True):
        super(UnoptimizedWelschNGF, self).__init__(fixed_image, moving_image, fixed_mask, moving_mask, size_average, reduce)

        self.isWelschEnabled = True
        self._name = "welsch_ngf"
        self._dim = fixed_image.ndim
    ...
    def _ngf_loss_2d(self, warped_image):
        _, self.value_last = self.parameters()
        ...
        value = F.pad(th.cat((dx, dy), dim=1) / norm, (0, 1, 0, 1))
        # self.value_last = value.clone().detach()
        #
        # return th.exp(-1 * self.inverse_2_vd_square * self.value_last) * value
        return value
    ...
    def _ngf_loss_3d(self, warped_image):
        ...
        value = F.pad(th.cat((dx, dy, dz), dim=1) / norm, (0, 1, 0, 1, 0, 1))
        # self.value_last = value.clone().detach()
        # # print("welschisoptv", self.inverse_2_vd_square)
        # return th.exp(-1 * self.inverse_2_vd_square * self.value_last) * value
        return value
    ...
    def forward(self, displacement):
        ...
        # compute current mask
        mask = super(UnoptimizedWelschNGF, self).GetCurrentMask(displacement)
        ...
        value = th.masked_select(1-value, mask)

        ngfs = value.clone().cpu().detach().numpy()
        y = np.mean(ngfs)
        self.init_vd = y
        self.vd = self.init_vd
        self.inverse_2_vd_square = 1 / (2 * self.vd ** 2)
...
#         self.value_last = value.clone().detach()
        ...
        value = 1-th.exp(-1*self.inverse_2_vd_square*value**2)
        if self.firstIter:
            self.firstIter = False
        return self.return_loss(value, welsch_k=1)
    
    
class WelschNGF(_PairwiseImageLoss):
    r""" Implementation of the Normalized Gradient Fields image loss.
    ...
    def __init__(self, fixed_image, moving_image, vd=0.8, fixed_mask=None, moving_mask=None, epsilon=1e-5,
                 size_average=True, reduce=True):
        super(WelschNGF, self).__init__(fixed_image, moving_image, fixed_mask, moving_mask, size_average, reduce)
...
#         _, self.value_last = self.parameters()
    ...
    def forward(self, displacement):
        ...
        # compute current mask
        mask = super(WelschNGF, self).GetCurrentMask(displacement)
        ...
        value = th.exp(-1 * self.inverse_2_vd_square * self.value_last**2) * value**2
        if self.firstIter:
            self.firstIter = False
        return self.return_loss(value, welsch_k=self.inverse_2_vd_square)
...
class MaskWelschNGF(_PairwiseImageLoss):
    r""" Implementation of the Normalized Gradient Fields image loss.
    ...
    def __init__(self, fixed_image, moving_image, vd=0.5, fixed_mask=None, moving_mask=None, epsilon=1e-5,
                 size_average=True, reduce=True):
        super(MaskWelschNGF, self).__init__(fixed_image, moving_image, fixed_mask, moving_mask, size_average, reduce)
        self.isWelschEnabled = True
        self._name = "ngf"
        self._dim = fixed_image.ndim
        self._epsilon = epsilon
        ...
        self.init_vd = vd
        self.vd = vd
        self.inverse_2_vd_square = 1 / (2 * vd ** 2)

        if self._dim == 2:
            dx = (self.fix[..., 1:, 1:] - self.fix[..., :-1, 1:]) * fixed_image.spacing[0]
            dy = (self.fix[..., 1:, 1:] - self.fix[..., 1:, :-1]) * fixed_image.spacing[1]
            ...
            dx = (self.fix[..., 1:, 1:, 1:] - self.fix[..., :-1, 1:, 1:]) * fixed_image.spacing[0]
            dy = (self.fix[..., 1:, 1:, 1:] - self.fix[..., 1:, :-1, 1:]) * fixed_image.spacing[1]
            dz = (self.fix[..., 1:, 1:, 1:] - self.fix[..., 1:, 1:, :-1]) * fixed_image.spacing[2]
    ...
    def forward(self, displacement):
        ...
        displacement = self._grid + displacement
        # compute current mask
        mask = super(MaskWelschNGF, self).GetMask(displacement)
        ...
        for dim in range(self._dim):
            value = value + ng_warped_image[:, dim, ...] * self._ng_fixed_image[:, dim, ...]
        # value = 0.5 * th.masked_select(-value.pow(2), mask)
        value = 0.5 * th.masked_select(1 - value.pow(2), mask)
...
class MaskNGF(_PairwiseImageLoss):
    r""" Implementation of the Normalized Gradient Fields image loss.
    ...
    def __init__(self, fixed_image, moving_image, fixed_mask=None, moving_mask=None, epsilon=1e-5,
                 size_average=True, reduce=True):
        super(MaskNGF, self).__init__(fixed_image, moving_image, fixed_mask, moving_mask, size_average, reduce)
    ...
    def forward(self, displacement):
        ...
        displacement = self._grid + displacement
        # compute current mask
        mask = super(MaskNGF, self).GetMask(displacement)
